...
function res.init()
	...
	print(ansicolors.cyan.."- Masks:"..ansicolors.clear)
	local files = love.filesystem.getDirectoryItems('res/masks')
	for i, b in ipairs(files) do
		if string.sub(b,-4) == '.png' then
			local item = {val = nil, users = 0}
			res.masks[b]=item
			print('Loaded mask, '..ansicolors.green..b..ansicolors.clear)
		end
	end
...
function res.load(typ, name,mode)
	if typ == 'image' and (res.images[name].users == 0 or res.images[name].val == nil) then
		-- if mode and mode == 'data' then
		-- 	if not res.images[name..'Data'] then
		-- 		res.images[name..'Data'] = {users = 0,val = nil}
		-- 	end
		-- 	res.images[name].users = res.images[name].users + 1
		-- 	res.images[name].val = love.image.newImageData('res/images/'..name)
		-- 	print(type(res.images[name].val))
		-- 	return res.images[name..'Data'].val
		-- else
			res.images[name].users = res.images[name].users + 1
	...
	elseif typ == 'mask' and (res.masks[name].users == 0 or res.masks[name].val == nil) then
		res.masks[name].users = res.masks[name].users + 1
		res.masks[name].val = love.image.newImageData('res/masks/'..name)
		return res.masks[name].val
