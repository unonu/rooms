...
function screen:drawFade()
	if self.timers.fade[1] ~= self.timers.fade[2] then
			self.timers.fade[1] = self.timers.fade[1] + 1
		if self.fadeDirection == 'in' then
			love.graphics.setColor(0,0,0,255-(self.timers.fade[1]/self.timers.fade[2])*255)
		elseif self.fadeDirection == 'out' then
			love.graphics.setColor(0,0,0,(self.timers.fade[1]/self.timers.fade[2])*255)
		end
		love.graphics.rectangle('fill',0,0,self.width,self.height)
	else
		self.timers.fade[1] = 0
		self.timers.fade[2] = 0
	end
end
...
function screen:fade(dir,len,col)
	if dir == 'in' or dir == 'out' then
		self.fadeDirection = dir
		self.timers.fade[1] = 0
		self.timers.fade[2] = len
	else
		print('unknown fade direciton')
	end
	if col then self.timers.fade[3] = col end
end

function  screen:setFadeColor(a1,a2,a3)
	if col then self.timers.fade[3] = {a1,a2,a3} end
end
...
function screen:newCamera(x,y,w,h,s,r,xx,yy)
	local camera = {}
	camera._x = xx or 0
	camera._y = yy or 0
	camera.x = x or 0
	...
	function camera:setPosition(x, y, m)
	  if x then self:setX(x) end
	  if y then self:setY(y) end
	  if m == 'c' then self:move(-self._width/2,-self._height/2) end
	end
	...
	function camera:getPosition(c)
		if c then return self._x+(self._width/2), self._y+(self._height/2)
		else return self._x, self._y end
	end
